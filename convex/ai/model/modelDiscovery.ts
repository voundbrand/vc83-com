...
export const getOpenRouterDiscoveryBinding = internalQuery({
  ...
  handler: async (ctx) => {
    const bindings = await (ctx as any).runQuery(
      generatedApi.internal.ai.model.modelDiscovery.getSystemDiscoveryBindings,
      {}
    );
  ...
  handler: async (ctx): Promise<{
    ...
    if (mergedModels.length === 0) {
      const cached = await (ctx as any).runQuery(
        generatedApi.internal.ai.model.modelDiscovery.getCachedModels,
        {}
      );
    ...
    await (ctx as any).runMutation(
      generatedApi.internal.ai.model.modelDiscovery.cacheModels,
      {
        models: mergedModels,
        fetchedAt,
...
export const getAvailableModels = internalQuery({
  ...
  handler: async (ctx): Promise<{
    ...
    const cached = await (ctx as any).runQuery(generatedApi.internal.ai.model.modelDiscovery.getCachedModels, {});
...
export const getModelsByProvider = query({
  ...
  handler: async (ctx): Promise<{
    ...
    const { models, isStale } = await (ctx as any).runQuery(
      generatedApi.internal.ai.model.modelDiscovery.getAvailableModels,
      {}
    );
...
export const refreshModels = action({
  ...
  handler: async (ctx): Promise<{models: OpenRouterModel[]; fetchedAt: number}> => {
    ...
    return await (ctx as any).runAction(generatedApi.internal.ai.model.modelDiscovery.fetchAvailableModels, {});
  },
});
...
export const backfillNativeReasoningCapabilities = action({
  ...
  handler: async (ctx, args) => {
    ...
    return await (ctx as any).runMutation(
      generatedApi.internal.ai.model.modelDiscovery.backfillNativeReasoningCapabilitiesInternal,
      {
