...
```bash
node --import tsx apps/one-of-one-landing/scripts/elevenlabs/sync-elevenlabs-agent.ts --agent anne_becker
```

Write:

```bash
node --import tsx apps/one-of-one-landing/scripts/elevenlabs/sync-elevenlabs-agent.ts --agent anne_becker --write
```
