      ...
      <main className="mx-auto max-w-7xl px-4 pt-24 pb-12 sm:px-6 lg:px-8">
        <div className="mb-8 space-y-4">
          <div>
            <h1 className="text-balance text-3xl font-bold tracking-tight text-foreground">
              Provisionen & Vermittlungen
            </h1>
            <p className="mt-1.5 text-muted-foreground">
              Attraktive Vermittlungsangebote mit Provisionen
            </p>
          </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
          <aside className="hidden lg:block">
            <SidebarFilters type="provisionen" />
          </aside>

          <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
            {provisionen.map((provision) => (
              <CommissionCard key={provision.id} provision={provision} />
            ))}
          </div>
        </div>
      </main>
