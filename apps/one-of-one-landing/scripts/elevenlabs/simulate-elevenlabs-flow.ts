...
async function main(): Promise<void> {
  ...
  for (const fixturePath of options.fixturePaths) {
    ...
    const analysis = analyzeConversation(fixture, conversation);
  ...
  return new Promise<SimulationRunResult>((resolveResult, rejectResult) => {
    ...
    ws.on("message", (rawMessage) => {
      ...
      if (eventType === "agent_tool_response") {
        ...
        lastActivityAt = Date.now();
        if (toolResponse.tool_name === "transfer_to_agent" && !toolResponse.is_error) {
        ...
        if (
          toolResponse.tool_name === "transfer_to_agent" &&
...
function analyzeConversation(
  ...
  if (fixture.assertions.forbidAgentSelfClaimMismatch) {
    for (const { turn } of speechTurns) {
      ...
      if (speaker && claimedAgent && claimedAgent !== speaker) {
        failures.push(
...
function printHelp(): void {
  console.log(`Usage:
  npx tsx apps/one-of-one-landing/scripts/elevenlabs/simulate-elevenlabs-flow.ts [--fixture clara-kai-clara-maren-regression] [--suite all-handoffs] [--all]

Examples:
  npx tsx apps/one-of-one-landing/scripts/elevenlabs/simulate-elevenlabs-flow.ts
  npx tsx apps/one-of-one-landing/scripts/elevenlabs/simulate-elevenlabs-flow.ts --fixture clara-kai-clara-maren-regression
  npx tsx apps/one-of-one-landing/scripts/elevenlabs/simulate-elevenlabs-flow.ts --suite all-handoffs
