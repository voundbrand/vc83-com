...
import { AGENT_KEYS, formatAgent, type LandingDemoAgentKey } from "./lib/catalog";
import {
  ElevenLabsClient,
...
async function main(): Promise<void> {
  ...
  for (const agentKey of options.agentKeys) {
    ...
    const remoteKnowledgeBase = source.knowledgeBase
      ...
      : undefined;
    const desiredPrompt = buildDesiredPrompt(remotePrompt, source.prompt, source.managedBuiltInTools);
...
function printHelp(): void {
  console.log(`Usage:
  npx tsx apps/one-of-one-landing/scripts/elevenlabs/sync-elevenlabs-agent.ts [--all] [--agent clara,kai] [--write]

Examples:
  npx tsx apps/one-of-one-landing/scripts/elevenlabs/sync-elevenlabs-agent.ts --all
  npx tsx apps/one-of-one-landing/scripts/elevenlabs/sync-elevenlabs-agent.ts --agent clara --write
