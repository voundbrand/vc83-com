...
- Do not add an app-local `vercel.json` unless this app later needs routing or header overrides.
- `apps/segelschule-altwarp/package.json` uses app-local scripts that call the shared root dependency tree.
...
- Install Command: `cd ../.. && npm ci --include=dev && npm --prefix apps/segelschule-altwarp ci`
- Ignored Build Step:
...
- Vercel default app-local install is not sufficient here because this app's scripts resolve `next` and `typescript` from the repo root `node_modules`.
- Root-only install is also not sufficient because this app has additional app-local dependencies in `apps/segelschule-altwarp/package-lock.json`.
