...
- system prompts from `convex/ai/agents/elevenlabs/landing-demo-agents/*/system-prompt.md`
- repo-managed first-message fields from `.../*/first-message.md` when an agent declares one
- agent knowledge bases from `convex/ai/agents/elevenlabs/landing-demo-agents/*/knowledge-base.md`
...
- client-owned agents from `convex/ai/agents/elevenlabs/client_owned/*` such as `anne-becker/*`
...
Agent catalog entries now declare lifecycle metadata:

- `active` (default sync/deploy subset): `Clara`, `Samantha`, `Veronica`, `Anne Becker`
- `inactive` (preserved assets, excluded by default): `Clara V3 Candidate`, `Maren`, `Jonas`, `Tobias`, `Lina`, `Kai`, `Nora`

Inactive agents are not deleted and keep their prompts/knowledge/tools in-repo; they are simply excluded from default sync runs unless explicitly selected.
...
- `CLARA_V3_ELEVENLABS_AGENT_ID` (required for `clara_v3`; no fallback id is used)
...
- `npm run ai:elevenlabs:sync` (repo root)
- `npm --prefix apps/one-of-one-landing run elevenlabs:sync` (app alias)
- `npm run landing:elevenlabs:sync` (legacy root alias)

Dry-run active subset (default):

```bash
npm run ai:elevenlabs:sync
```

Dry-run full catalog (including inactive specialists):

```bash
npm run ai:elevenlabs:sync -- --all
```
...
Write Clara V3 candidate only (requires `CLARA_V3_ELEVENLABS_AGENT_ID`):

```bash
npm run ai:elevenlabs:sync -- --agent clara_v3 --write
```

`clara_v3` is intentionally configured as intake-only:
- no specialist `transfer_to_agent` tool
- no specialist workflow graph
- only shared `end_call` and `transfer_to_number` tools

Write multiple agents:

```bash
npm run ai:elevenlabs:sync -- --agent clara,kai,maren --write
```
...
```bash
npm run ai:elevenlabs:test-sync -- --all --write
```

Dry-run the specialist inventory without writing:

```bash
npm run ai:elevenlabs:test-sync -- --all
```
...
```bash
npm run ai:elevenlabs:simulate -- --fixture clara-kai-clara-maren-regression
```

Run the full handoff battery:

```bash
npm run ai:elevenlabs:simulate -- --suite all-handoffs
```

Run the proof-phase gating path:

```bash
npm run ai:elevenlabs:simulate -- --suite proof-phase-gating
```

Run the exploratory proof stress path:

```bash
npm run ai:elevenlabs:simulate -- --suite proof-phase-stress
```

List available fixtures:

```bash
npm run ai:elevenlabs:simulate -- --list-fixtures
```

List built-in suites:

```bash
npm run ai:elevenlabs:simulate -- --list-suites
```

Useful knobs:

```bash
npm run ai:elevenlabs:simulate -- --fixture clara-kai-clara-maren-regression --idle-ms 1800 --handoff-settle-ms 3000 --turn-timeout-ms 25000
```
