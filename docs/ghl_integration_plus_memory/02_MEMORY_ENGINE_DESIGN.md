...
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  Layer 1: RECENT CONTEXT (Last 10-15 messages verbatim)            │
│  ┌────────────────────────────────────────────────────────────┐    │
...
│  └────────────────────────────────────────────────────────────┘    │
│                                                                      │
│  Layer 2: SESSION SUMMARY (Auto-generated every 10 messages)        │
│  ┌────────────────────────────────────────────────────────────┐    │
...
│  └────────────────────────────────────────────────────────────┘    │
│                                                                      │
│  Layer 3: OPERATOR PINNED NOTES (Human-curated strategy) ★ NEW     │
│  ┌────────────────────────────────────────────────────────────┐    │
...
│  └────────────────────────────────────────────────────────────┘    │
│                                                                      │
│  Layer 4: CONTACT PROFILE (Structured facts, AI-extracted)          │
│  ┌────────────────────────────────────────────────────────────┐    │
...
│  └────────────────────────────────────────────────────────────┘    │
│                                                                      │
│  Layer 5: REACTIVATION CONTEXT (Triggered when > 7 days idle)       │
│  ┌────────────────────────────────────────────────────────────┐    │
...
## Layer 3: Operator Pinned Notes (NEW — Human-Curated Strategy)
...
1. AI automation (Layers 1, 2, 4, 5)
2. Human intelligence (Layer 3)
3. Structured data (Layer 4)

---

## Layer 1: Recent Context Window (Unchanged)
...
### **1. Session Start**
- Load recent context (Layer 1) ✅
- Load operator pinned notes (Layer 3) ✅
- Load contact profile (Layer 4) if contact is known ✅
- Check for reactivation (Layer 5) ✅

### **2. During Conversation**
- Every message: Add to recent context
- Every 10 messages: Trigger summarization (Layer 2)
- After each response: Check if extraction needed (Layer 4)
- Operator can add pinned notes (Layer 3) at any time

### **3. Session Idle (24h+)**
- Generate final summary (Layer 2)
- Generate reactivation context (Layer 5) for next time
...
- Detect gap > 7 days
- Load operator pinned notes (Layer 3)
- Inject reactivation context (Layer 5)
- Resume normal flow

### **5. Cross-Channel Handoff**
- If same contact messages via different channel
- Merge into existing session OR
- Create new session but link to same contact (unified Layer 4)
- Operator notes (Layer 3) follow the contact across channels
...
async function buildMemoryContextLazy(
  ...
  // Priority 3: Include Layer 2 (summary) if space allows
  ...
  // Priority 4: Include Layer 5 (reactivation) if applicable and space allows
...
**Next:** [03_GHL_API_INTEGRATION.md](./03_GHL_API_INTEGRATION.md) — Webhook and API implementation details
