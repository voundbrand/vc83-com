...
| `WCV-203` | `C` | `MP §Three-phase plan / Phase 3` | `P0` | `READY` | `WCV-202` | Migrate web chat runtime orchestration to persistent session path with deterministic fallback to current turn-stitch mode. | `src/hooks/use-voice-runtime.ts`; `src/components/window-content/ai-chat-window/slick-pane/slick-chat-input.tsx`; `convex/ai/chatRuntimeOrchestration.ts` | `V-TYPE`; `V-E2E-DESKTOP`; `V-DOCS` | Preserve interruption and transcript continuity contracts. Dependency `WCV-202` satisfied. |
