...
|---|---|---|---|---|
| Partial transcript latency `P95` | `> 900ms` for 2 consecutive windows | 10m rolling; evaluate every 5m | Freeze percentage increase; page runtime + mobile owners | Roll back to prior stable mobile rollout percent and force `websocket`-first fallback while incident is open |
| Barge-in interrupt latency `P95` | `> 700ms` for 2 consecutive windows | 10m rolling; evaluate every 5m | Stop canary promotion; mark `DEGRADED` in runtime status board | Roll back canary cohort and disable realtime assistant streaming (`chunked_fallback` only) |
