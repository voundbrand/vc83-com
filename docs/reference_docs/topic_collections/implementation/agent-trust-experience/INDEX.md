...
18. `ATX-016` (`DONE`): implemented thread-level control-center optics with canonical lifecycle badges, separate delivery-state badges, escalation/active-instance badges, waiting-on-human-first sorting, and an org-level waiting-on-human header indicator.
