      ...
      {showProgressModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.7)' }}>
          <div className="w-full max-w-2xl border-4 shadow-[8px_8px_0px_0px_rgba(0,0,0,0.8)]" style={{ borderColor: 'var(--win95-border)', background: 'var(--win95-bg-light)' }}>
                ...
                <div className="space-y-2">
                  ...
                  {executionLogs.logs.map((log, index) => (
                    <div
                      ...
                      style={{
                        borderColor: 'var(--win95-border)',
                        background: log.level === 'error' ? 'var(--error-light)' : log.level === 'success' ? 'var(--success-light)' : log.level === 'warning' ? 'var(--warning-light)' : 'var(--win95-bg)'
                      }}
                    >
                      <div className="flex items-start gap-2">
                        <div className="mt-0.5">
                          {log.level === 'error' ? (
                            <div className="h-4 w-4 rounded-full border-2 flex items-center justify-center text-xs font-bold" style={{ borderColor: 'var(--error)', color: 'var(--error)' }}>✕</div>
                          ) : log.level === 'success' ? (
                            <CheckCircle2 className="h-4 w-4" style={{ color: 'var(--success)' }} />
                          ) : log.level === 'warning' ? (
                            <Clock className="h-4 w-4" style={{ color: 'var(--warning)' }} />
                          ) : (
                            <div className="h-4 w-4 rounded-full border-2" style={{ borderColor: 'var(--neutral-gray)' }} />
                          )}
                        </div>
                        <div className="flex-1">
            ...
            {/* Footer */}
            <div className="border-t-4 px-4 py-3 flex items-center justify-between gap-2" style={{ borderColor: 'var(--win95-border)' }}>
              <div className="flex items-center gap-2">
                {createdInvoiceId && (
                  <button
                    onClick={() => {
