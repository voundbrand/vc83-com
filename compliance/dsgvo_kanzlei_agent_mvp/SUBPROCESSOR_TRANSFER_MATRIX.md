...
## Verfuegbares Vertrags-Basispaket

Als juristisches Intake-Artefakt ist zusaetzlich vorhanden:

- `/Users/foundbrand_001/Development/vc83-com/docs/strategy/one-of-one-v4/existing-avvs-docs/`

Hinweis:

1. Das Basispaket enthaelt AVV/SaaS/SLA + Anlagen und ist fuer Klausel-/Scope-Mapping nutzbar.
2. Die Matrix bleibt trotzdem auf `unknown`/`pending_review`, bis provider-spezifische Nachweise je Row verlinkt sind.
...
|---|---|---|---|---|---|---|---|---|---|---|
| Convex | Backend/DB | `convex/*`, `src/components/providers/convex-provider.tsx`, `NEXT_PUBLIC_CONVEX_URL` usage | App backend, data persistence, queries/mutations | Account, org, workflow, content, logs | Snapshot 2026-03-26 (post-migration): EU target project `sevelayers-eu` uses prod `dashing-cuttlefish-674` + dev `handsome-trout-897` in `aws-eu-west-1`; legacy `agreeable-lion-828` remains historical | `unknown` | `unknown` | Engineering + Datenschutz | 2026-04-24 | `pending_review` |
...
| Vercel Analytics (app-specific) | Analytics/hosting telemetry | `apps/segelschule-altwarp/app/layout.tsx` (`@vercel/analytics/next`) | Web analytics for specific app surfaces | Page interaction metadata | Snapshot 2026-03-26 (post-redeploy): tracked linked projects (`vc83-com`,`segelschule-altwarp`,`guitarfingerstyle`) on `cdg1/dub1/fra1` | `unknown` | `unknown` | App owner + Datenschutz | 2026-04-24 | `pending_review` |
...
4. Convex region evidence references:
...
5. Vercel region evidence references:
...
1. `TODO-KAMVP-007-001`: Confirm final environment-cutover consistency so active production surfaces no longer reference legacy `agreeable-lion-828`.
